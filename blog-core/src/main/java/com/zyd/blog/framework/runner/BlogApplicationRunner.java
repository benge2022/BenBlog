...
public class BlogApplicationRunner extends ContextLoaderListener implements ApplicationRunner {
    ...
    public void contextInitialized(ServletContextEvent event) {
        ...
        if (this.printMyLogFlag()){
            this.printMyLog();
        } else if (appProperties.isEnabledPrintConfig()) {
            log.info("博客关键配置信息：");
    ...
    private boolean printMyLogFlag(){
        return true;
    }

    private void printMyLog(){
        if (appProperties.isEnabledPrintConfig()) {
            ...
            if (ObjectUtils.isEmpty(activeProfiles)) {
                ...
                // 被加载的配置文件
                System.out.println("被加载的配置文件 active: " + StringUtils.arrayToCommaDelimitedString(activeProfiles));
                // log.info("The following profiles are active: " + StringUtils.arrayToCommaDelimitedString(activeProfiles));
            }

            System.out.println("DB Source：");
            System.out.println(String.format("{username: '%s', password: '%s', url:'%s'}"
                    , dataSourceProperties.getPassword()
                    , dataSourceProperties.getUsername()
                    , dataSourceProperties.getUrl()));

            System.out.println("Redis：");
            System.out.println(String.format("{database: '%s', host: '%s', port:'%s', password:'%s'}"
